...
struct
  ...
  let init_state = ([], [])
  let rec next_state c s = match c with
    | Pop ->
      (match s with
        | ([], []) -> failwith "tried to pop empty queue"
        | ([], s1') -> next_state c (List.rev s1', [])
        | (_::s0', s1') -> (s0', s1')
      )
    | Top -> s
    | Push i -> 
  ...
  let cleanup _   = ()
  let rec run_cmd c s q = match c with
    | Pop -> (try match s with
                | ([], []) -> failwith "tried to pop empty queue"
                | ([], s1') -> run_cmd c (List.rev s1', []) q
                | (s0', _) -> Queue.pop q = List.hd s0'
              with _ -> false)
    | Top -> (try match s with
                | ([], []) -> failwith "tried to top empty queue"
                | ([], s1') -> run_cmd c (List.rev s1', []) q
                | (s0', _) -> Queue.top q = List.hd s0'
              with _ -> false)
    | Push n -> begin Queue.push n q; true end (* thinks queue is arbitrary instead of int due to something in the pop/top match *)
    | Length -> Queue.length q = List.length (fst s) + List.length (snd s)
